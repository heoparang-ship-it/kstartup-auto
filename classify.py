#!/usr/bin/env python3
"""
K-Startup 공고 자동 분류기 v8.4 — 3티어 전원 수용 + 1순위 정밀화 + 감사 루틴
═════════════════════════════════════════════════════════════════════════════

v8.4 변경점 (2026-04-19 주간 감사):
- 주간 green 감사 결과 10건 적출 → RED_* 키워드 보강
...
- 적출된 10건: 176801(프리팁스 비수도권한정), 176718(아산두어스 20대 한정),
  177174(ESG 대중견기업·녹색융합클러스터 한정), 176958(DIPS 붙임1 명단 한정),
  177101(인천콘텐츠코리아랩 캐릭터 콘텐츠 한정), 176179(전략기술 대학·출연연 한정),
  177048(사회적기업 한정), 176940(소상공인 한정),
  176076(TIPS 운영사 추천 선행조건), 175783(통합공고 안내성)
...
RED_INDUSTRY = [
    ...
    # v8.4: 특수 콘텐츠 서브도메인 (일반 콘텐츠 사업은 통과하되 세부 업종만 차단)
    "캐릭터 콘텐츠",
]
RED_QUALIFICATION = [
    ...
    # v8.4: 주간 감사 보강 (지역·연령·기관·업종·선행조건 한정)
    "비수도권 소재",          # 프리팁스류 — 인천(수도권) 자격 없음
    "20대 예비창업",           # 아산 두어스류 — 연령 한정
    "20대 창업가",             # 아산 두어스류 — 연령 한정
    "녹색융합클러스터",         # ESG 오픈이노베이션 — 클러스터 입주기업만
    "대·중견기업 및 기관",      # ESG 오픈이노베이션 — 대중견 한정
    "대학/출연연 소속",         # 전략기술 딥테크 — 출연연 한정
    "공공기술 활용 사업화",     # 전략기술 딥테크 — 공공기술 보유자만
    "예비사회적기업",           # 사회적기업 시설·운영비 — 인증 사회적기업만
    "사회적협동조합",           # 사회적기업 시설·운영비 — 사회적협동조합만
    "팁스 운영사로부터",        # TIPS 본공고 — 운영사 추천 선행 필요
    "소상공인시장진흥공단",     # 소상공인 립스 — 소상공인 한정
]
RED_STAGE = [
...
RED_NATURE = [
    ...
    # v8.4: 주간 감사 보강 (선행참여·안내성 공고)
    "[붙임1] 상 사업 명단",   # DIPS Link-up — 선행 사업 명단 한정
    "사업 명단에 참여한",      # DIPS Link-up — 선행 사업 명단 한정
    "각 지원사업 모집 공고문 참고",  # 통합공고 — 안내성, 실제 신청 사업 아님
]
