...
async function sortItems(items) {
  ...
  // 즐겨찾기 최상단 고정 → 그다음 점수순 (재정렬 시에도 ★ 위치 유지)
  return [...items].sort((a, b) => {
    const fa = _currentFavs.has(a.id) ? 1 : 0;
    const fb = _currentFavs.has(b.id) ? 1 : 0;
    if (fa !== fb) return fb - fa;
    return (SCORE_CACHE.get(b.id)?.score ?? 0) - (SCORE_CACHE.get(a.id)?.score ?? 0);
  });
}
